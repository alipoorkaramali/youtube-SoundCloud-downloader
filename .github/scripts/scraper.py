...
class TelegramChannelScraper:
    ...
    async def _fetch_posts_from_telegram(self) -> tuple[List[Dict], any, any]:
            ...
            for sel in scroll_button_selectors:
                try:
                    ...
                    if await btn.count() > 0:
                        ...
                        self.logger.info("   ✅ روی دکمهٔ فلش کلیک شد.")
                        clicked = True
                    ...
                    continue
            if not clicked:
                self.logger.info("   ℹ️ دکمهٔ پرش پیدا نشد.")
        else:
        ...
        if self.start_link and self.target_msg_id:
            self.logger.info(f"🎯 پیدا کردن پیام هدف {self.target_msg_id} و بردن به بالای صفحه...")
            try:
        ...
        while len(items) < self.limit and scroll_attempts < MAX_SCROLL_ATTEMPTS:
                    ...
                    try:
                        ...
                        if self.start_link and not start_collecting:
                            if msg_id == self.target_msg_id:
                                ...
                                self.logger.info(f"🎯 به پیام هدف رسیدیم (ID: {msg_id})")
                            else:
                                continue
            ...
            if self.start_link and not start_collecting:
                self.logger.info("🔄 هنوز به پیام هدف نرسیدیم، اسکرول بیشتر...")
    ...
    # ======================== جستجو و ورود به کانال (برای حالت بدون لینک) ========================
    async def _search_and_enter_channel(self, page) -> bool:
        search_input = None
        for sel in ['input[placeholder*="Search"]', 'input[role="textbox"]', '[data-testid="search-input"]']:
            try:
        ...
        if not found:
            ...
            if await self._check_text_on_page(page, search_term):
                found = True
        if not found:
            self.logger.info("   📑 کلیک روی تب Channels ...")
            try:
        ...
        if not found:
            self.logger.error(f"❌ نتایج جستجو برای '{search_term}' پیدا نشد.")
    ...
    # ======================== متد جستجو با لینک (اصلاح‌شده طبق شفاف‌سازی) ========================
    async def _navigate_to_start_link(self, page) -> bool:
        ...
        # ۳. انتظار برای بارگذاری اولیه
        await human_sleep(5, 0.5)
        await self._take_screenshot(page, "search_results_loaded")

        # ۴. پیدا کردن لینک (a) با href حاوی شناسه پیام
        link_selector = f'a[href*="/{self.target_msg_id}"]'
        self.logger.info(f"🔗 جستجوی لینک با href شامل '{self.target_msg_id}'")
        try:
            await page.wait_for_selector(link_selector, timeout=10000)
            self.logger.info("✅ لینک پیام هدف پیدا شد.")
        except Exception:
            self.logger.warning("⚠️ لینک پیام هدف پیدا نشد. تلاش fallback...")
            return await self._fallback_click_result(page)

        message_link = page.locator(link_selector).first
        try:
            await message_link.scroll_into_view_if_needed()
            # هایلایت (اختیاری)
            try:
                ...
                }''', await message_link.element_handle())
                await human_sleep(1, 0.3)
                await self._take_screenshot(page, "before_click_link")
            except Exception as e:
                self.logger.debug(f"خطا در هایلایت: {e}")

            await message_link.click(timeout=5000, force=True)
            self.logger.info("✅ کلیک روی لینک پیام هدف انجام شد.")
        except Exception as e:
            self.logger.error(f"❌ کلیک روی لینک پیام هدف ناموفق: {e}")
            await self._take_screenshot(page, "click_link_failed")
            return False

        # ۵. منتظر بارگذاری صفحه پیام
        ...
        try:
            ...
            self.logger.info("✅ صفحه پیام هدف بارگذاری شد.")
            await self._take_screenshot(page, "message_page_loaded")
            return True
        except Exception:
            self.logger.warning("⚠️ پیام هدف در دید نیست، اسکرول سریع...")
            for i in range(5):
                await page.evaluate("window.scrollBy(0, -3000)")
                await human_sleep(2, 0.5)
                try:
                    await page.wait_for_selector(f'div[data-message-id="{self.target_msg_id}"]', timeout=5000)
                    self.logger.info("✅ پیام هدف پس از اسکرول پیدا شد.")
                    return True
                except Exception:
                    continue
    ...
    async def _fallback_click_result(self, page) -> bool:
        """روش fallback: کلیک روی اولین نتیجه زیر بخش Messages"""
        self.logger.info("🔄 fallback: جستجوی بخش 'Messages' و کلیک روی اولین آیتم...")
        try:
            # کلیک روی تب Messages اگر وجود دارد
        ...
        # حالا اولین نتیجه را پیدا کرده و کلیک می‌کنیم
        result_selectors = [
            'div[data-message-id]',
        ...
        for sel in result_selectors:
            try:
                ...
                if await first.count() > 0:
                    ...
                    self.logger.info(f"✅ (fallback) کلیک روی '{sel}'")
                    await human_sleep(5, 0.5)
                    if await page.locator('div[data-message-id]').count() > 0:
                        return True
            except Exception:
                continue
        self.logger.error("❌ fallback هم موفق نشد.")
        ...
        for idx, item in enumerate(items):
            ...
            try:
                ...
                if await locator.count() == 0:
                    self.logger.warning(f"⚠️ المان پست {msg_id} پیدا نشد.")
                    continue
